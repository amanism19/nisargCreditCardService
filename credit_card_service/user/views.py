...
class RegisterUserView(APIView):
    ...
    def handle_exception(self, exc):
        if isinstance(exc, KeyError):
            return Response(data='Invalid data: ' + str(exc), status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        ...
        return Response(data={'user_id': str(user.user_id)}, status=status.HTTP_201_CREATED)


class ApplyLoanView(APIView):
    def validate_data(self, data):
        return (
            data['user_id'], data['loan_type'], data['loan_amount'],
            data['interest_rate'], data['term_period'], data['disbursement_date']
        )

    def handle_exception(self, exc):
        if isinstance(exc, (KeyError, ObjectDoesNotExist, ValueError)):
            return Response(data='Invalid data: ' + str(exc), status=status.HTTP_400_BAD_REQUEST)
        return super().handle_exception(exc)
    ...
    def post(self, request):
        ...
        if cumulative_interest <= 50:
            raise ValueError('Cumulative Interest is too low')
        ...
        for _ in range(term_period):
            ...
            interest_accrued = round(interest_rate / 365, 3) * days_of_interest * current_principal_balance / 100 
            due_date = billing_date + timedelta(days=15)

            emi_amount = round(loan_amount / term_period + interest_accrued)
            payment = Payment(
                loan=loan,
            ...
            billing_date = datetime(new_year, new_month, user.billing_day)
            payment.save()
        
        response_data = {'loan_id': str(loan.loan_id), 'due_dates': due_dates}
